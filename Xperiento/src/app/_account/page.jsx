...
const Index = () => {
    ...
    const { auth, signInHandler } = useContext(UserContext)
    ...
    const onSubmit = async (data) => {
        const { email, password, businessName, phoneNumber, firstName, lastName } = data;
        ...
        if (!isCreatingAccount) {
            ...
            res = await signUp({ firstName, lastName, email, password, phoneNumber, industrySegment: businessName, orgnazation: businessType })
        }
        const parseData = res.data || {success:false,data:"testing"}
        if (parseData.success) {
            toast.success(isCreatingAccount ? "Account Created Successfully" : "Logged In. Redirecting to the Dashboard... ")
    ...
    return (
        <>
            <form onSubmit={handleSubmit(onSubmit)}>
                <h3>
                    {isCreatingAccount == false ? "New Insights almost every day!" : "Complete Your Profile for the right insights."}
                </h3>
                {isCreatingAccount && (
                    <>
                        <div data-state={isCreatingAccount} className="flex-column">
                            ...
                            <input {...register("firstName", {
                                ...
                                pattern: {
                                    value: /^[A-Z][a-zA-Z0-9]*$/,
                                    message: "First Name must start with an uppercase letter and can only contain letters (uppercase and lowercase) and digits"
                                }
                            })}
                                id="firstName" type="text" />
                            {errors.firstName && <span style={{ fontSize: ".7rem", color: "red" }}>{errors.firstName.message || "Validation Error"}</span>}
                        </div>
                        <div data-state={isCreatingAccount} className="flex-column">
                            ...
                            <input {...register("lastName", {
                                ...
                                pattern: {
                                    value: /^[A-Z][a-zA-Z0-9]*$/,
                                    message: "Last Name must start with an uppercase letter and can only contain letters (uppercase and lowercase) and digits"
                                }
                            })}
                                id="lastName" type="text" />
                            {errors.lastName && <span style={{ fontSize: ".7rem", color: "red" }}>{errors.lastName.message || "Validation Error"}</span>}
                        </div>
                        <div data-state={isCreatingAccount} className="flex-column">
                        ...
                        <div data-state={isCreatingAccount} className="flex-column">
                            ...
                            {errors.businessName && <span style={{ fontSize: ".7rem", color: "red" }}>{errors.businessName.message || "Validation Error"}</span>}
                        </div>
                ...
                <div className="flex-column">
                    ...
                    <input
                        {...register("phoneNumber", {
                            ...
                            pattern: {
                                value: /^\d{10,12}$/,
                                message: "Phone number must be between 10 to 12 digits and contain only numbers"
                            },
                            minLength: {
                                value: 10,
                        ...
                        type="number"
                    />
                    {errors.phoneNumber && <span style={{ fontSize: ".7rem", color: "red" }}>{errors.phoneNumber.message}</span>}
                </div>
                <div className="flex-column">
                    ...
                    {errors.password && <span style={{ fontSize: ".7rem", color: "red" }}>{errors.password.message || "Validation Error"}</span>}
                </div>

                {isCreatingAccount &&
                    <div data-state={isCreatingAccount} className="radio-btn">
                        <svg onClick={() => { setBusinessType("Restaurant") }} width="70" height="70" viewBox="0 0 89 89" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <svg onClick={() => { setBusinessType("Shopping") }} width="70" height="70" viewBox="0 0 89 89" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {loading ? <button className='start' type='button' disabled={true}> <i style={{ fontSize: "1.5rem" }} className="pi pi-spin pi-spinner"></i></button>
                    : <button className='start' type='submit'>{isCreatingAccount ? 'Create Account' : 'Log In'}</button>}
                <button onClick={toggleForm} type='button' className="link">{!isCreatingAccount ? "Create Account" : "Login Account"}</button>
            </form >
        </>
